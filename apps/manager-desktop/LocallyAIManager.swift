// LocallyAI Manager — minimal macOS desktop wrapper around the
// firm's Manager UI. Replaces "open the URL in Safari" with a real
...
//   1. LOCALLYAI_MANAGER_URL environment variable (runtime override)
//   2. UserDefaults key "ManagerURL" (set once via first-run dialog)
...
#else
let COMPILE_TIME_DEFAULT_URL = "https://office-mac.local:8000"
#endif
...
final class WindowController: NSObject, WKNavigationDelegate, WKUIDelegate, WKDownloadDelegate {
    ...
    init(url: URL) {
        ...
        window.title = "LocallyAI Manager"
    ...
    // window.open handling — same shape as Workspace.app. Without
    // this WebKit silently drops window.open() calls and the
    // "Open document" / "View report in new window" buttons appear
    // dead.
    private var _childWindows: [NSWindow] = []
    ...
    func webView(_ webView: WKWebView,
        ...
        let screen = NSScreen.main?.visibleFrame ?? NSRect(x: 0, y: 0, width: 1440, height: 900)
        let w: CGFloat = 1100; let h: CGFloat = 800
        let win = NSWindow(
    ...
    // Manager UI (compliance snapshot, audit CSV, installer zips).
    //
...
enum ManagerURL {
    static func resolve() -> URL {
        ...
        if let raw = ProcessInfo.processInfo.environment["LOCALLYAI_MANAGER_URL"],
           let url = URL(string: raw), url.scheme != nil {
            return url
        }
        // 2. UserDefaults (set by first-run dialog or operator)
        if let raw = UserDefaults.standard.string(forKey: "ManagerURL"),
           let url = URL(string: raw), url.scheme != nil {
            return url
        }
    ...
    static func set(_ urlString: String) {
        UserDefaults.standard.set(urlString, forKey: "ManagerURL")
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func installMenu() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "About LocallyAI Manager", action: #selector(NSApplication.orderFrontStandardAboutPanel(_:)), keyEquivalent: "")
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(withTitle: "Set Manager URL…", action: #selector(promptForURL), keyEquivalent: ",").target = self
        appMenu.addItem(NSMenuItem.separator())
        appMenu.addItem(withTitle: "Hide LocallyAI Manager", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
    ...
    @objc func promptForURL() {
        let alert = NSAlert()
        alert.messageText = "LocallyAI Manager URL"
        ...
        input.stringValue = ManagerURL.resolve().absoluteString
