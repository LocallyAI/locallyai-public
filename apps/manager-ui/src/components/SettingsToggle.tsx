...
// Drops into the TopBar — between the firm pill and the system-status
// pills works well. Closes on outside click.
//
// Language change reloads the window so every t() consumer picks up
// the new strings. Theme change is a CSS-only class flip on <html>.
  ...
  return (
    <div ref={wrapRef} className={cn("relative", className)}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex h-7 w-7 items-center justify-center rounded-md border border-border bg-card text-muted-foreground hover:text-foreground hover:bg-accent"
      ...
      {open && (
        <div
          role="menu"
          className="absolute end-0 top-full mt-2 w-56 rounded-md border border-border bg-popover p-1 text-popover-foreground shadow-lg z-50"
        >
