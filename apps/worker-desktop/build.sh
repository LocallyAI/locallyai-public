#!/usr/bin/env bash
# Build LocallyAI Workspace.app — a minimal macOS WKWebView wrapper
# around the firm's Manager UI. Output: dist/LocallyAI Workspace.app
# (and a zip alongside for distribution).
#
# Usage:
#   ./build.sh                                  # default URL baked
#   WORKSPACE_URL=https://office.local:8000 ./build.sh
#
# The URL ALSO defaults to compile-time, but is overridable at runtime
# via env var LOCALLYAI_WORKSPACE_URL or via the "Set Manager URL…" menu
...
WORKSPACE_URL="${WORKSPACE_URL:-http://office-mac.local:5174}"
APP_NAME="LocallyAI Workspace"
BUNDLE_ID="app.locallyai.worker-desktop"
...
WORK_DIR="$(mktemp -d)"
WORK_SRC="$WORK_DIR/LocallyAIWorkspace.swift"
trap 'rm -rf "$WORK_DIR"' EXIT
sed "s|http://office-mac.local:5174|${WORKSPACE_URL}|g" LocallyAIWorkspace.swift > "$WORK_SRC"
...
swiftc \
    ...
    -framework WebKit \
    "$WORK_SRC" \
    -o "$OUT/Contents/MacOS/LocallyAIWorkspace"
